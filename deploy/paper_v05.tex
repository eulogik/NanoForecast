...
\item We provide a complete production deployment pipeline including ONNX export (~13~MB FP16, ~6.5~MB INT8), Docker containers, and streaming inference, enabling training on a single cloud GPU (T4, Google Colab) and inference on edge hardware (\$35 USD).
\end{enumerate}
...
\item \textbf{Model export}: ONNX export produces a ~13\,MB model in FP16 (~6.5\,MB with INT8 quantization).
