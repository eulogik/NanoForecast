...
Our primary contribution is empirical rather than architectural. We demonstrate that systematic training pipeline refinement can yield substantial accuracy improvements without model architecture changes. Starting from the released NanoForecast v0.3 checkpoint (MASE 3.030 under the standard protocol of Section~\ref{sec:setup}, 6.5M parameters), we identify three issues in the training pipeline---loss-scope handling, tensor shape alignment, and augmentation coverage---and retrain the identical architecture with the corrected pipeline to obtain v0.5 (MASE 1.704), a 43.8\% improvement.

Additionally, we show that a 6.5M parameter model can outperform models 31$\times$ larger on specific benchmarks: it outperforms TimesFM (200M parameters) on four of six benchmarks---the three ETT datasets (ETTh1, ETTh2, ETTm1) plus exchange rate---and it outperforms PatchTST (15M+ parameters) on all three ETT sets.
...
\item We achieve a 43.8\% MASE improvement (3.030 $\to$ 1.704) through training pipeline refinement alone, with no architecture modifications.
\item We demonstrate that a 6.5M parameter model can outperform models 31$\times$ larger on specific benchmarks: outperforming TimesFM (200M parameters) on four of six benchmarks (ETTh1 MASE 0.676 vs.\ 0.705, ETTh2 1.110 vs.\ 1.360, ETTm1 0.287 vs.\ 0.545, exchange rate 4.317 vs.\ 4.383), while outperforming PatchTST (15M+ parameters) on all three ETT sets.
\item We provide a complete production deployment pipeline including ONNX export (27.9\,MB FP32, 9.2\,MB INT8), Docker containers, and stateful streaming inference, enabling training on a single cloud GPU (T4, Google Colab) and CPU-only inference (measured: 19.5\,ms per forecast on an Apple M4; ONNX Runtime 10.7\,ms).
\end{enumerate}
...
\draw[arrow] (base) -- node[label, above] {43.8\%} (fix3);
...
The three fixes were validated jointly; the released v0.3 and v0.5 checkpoints bracket the joint effect (Table~\ref{tab:ablation}), evaluated under the same protocol used for every model in this paper.
...
The ETT datasets use the standard train/validation/test temporal splits of 70\%/20\%/10\%; Exchange, Electricity, and Traffic use the standard 7:1:2 splits (70\%/10\%/20\%).
...
\textbf{Training.} 200 epochs, batch size 128, OneCycleLR scheduler (base learning rate $3 \times 10^{-5}$, peak $3 \times 10^{-4}$, 10\% warmup, cosine anneal), AdamW optimizer (weight decay $\lambda=0.01$), gradient clipping at 1.0, seed 42. Trained on a single NVIDIA T4 GPU (Google Colab) in approximately 12 hours per run (v0.3: 11.7\,h, v0.5: 12.2\,h recorded wall time). Released checkpoints are the validation-best snapshot: epoch 147 for v0.3, epoch 51 for v0.5 (best validation loss 0.2230 vs.\ 0.2204, both under the same loss family).

\textbf{Evaluation.} For all NanoForecast checkpoints (v0.3 and v0.5 alike), the point forecast is the pinball-trained median ($p_{50}$) from the quantile head rather than the MSE-blended point-head output: the conditional median is the MAE-optimal predictor, matching the MAE-based MASE metric. This inference-time choice is applied identically to both checkpoints, so all comparisons are unaffected. We report MASE (Eq.~\ref{eq:mase}) as the metric. For each series, the test segment is partitioned into non-overlapping windows of length $H$; the input context is the 512 observations immediately preceding each window. Per-window errors are averaged within a series, then across series. Every reported number for every model---including all baselines---was produced by us under this identical protocol, and the full per-dataset results are released with the evaluation framework.

\textbf{Compared methods.} We compare against TimesFM~\cite{timesfm} (200M parameters) and PatchTST~\cite{patchtst} (15M+), as well as previous NanoForecast versions (v0.3: 6.5M). Baselines use the official released checkpoints: PatchTST is trained per dataset with the official implementation and hyperparameters; TimesFM uses \url{https://huggingface.co/google/timesfm-1.0-200m-pytorch}. Chronos-T5-large~\cite{chronos} (up to 710M) and Timer~\cite{timer} are discussed qualitatively; they are not included in Table~\ref{tab:main} because inference was intractable on our hardware for the large datasets, and we include only models whose official releases permit reproducible evaluation under a single protocol.
...
\begin{itemize}
\item \textbf{43.8\% improvement} over v0.3 (standard-protocol MASE 3.030 $\to$ 1.704, Table~\ref{tab:ablation}) through training pipeline refinement alone.
\item \textbf{Outperforms TimesFM on four of six benchmarks}---all three ETT sets (ETTh1 0.676 vs.\ 0.705, ETTh2 1.110 vs.\ 1.360, ETTm1 0.287 vs.\ 0.545) plus exchange rate (4.317 vs.\ 4.383)---despite using 31$\times$ fewer parameters.
\item \textbf{Loses on high-cardinality sets}: electricity (2.029 vs.\ 0.923) and traffic (1.805 vs.\ 0.765), where TimesFM's breadth of pretraining dominates.
...
\midrule
v0.3 (original pipeline) & 0.681 & 1.328 & 0.288 & 11.758 & 2.213 & 1.913 & 3.030 \\
v0.5 (all three fixes) & 0.676 & 1.110 & 0.287 & 4.317 & 2.029 & 1.805 & 1.704 \\
$\Delta$ (lower better) & $-0.7\%$ & $-16.4\%$ & $-0.2\%$ & $-63.3\%$ & $-8.3\%$ & $-5.7\%$ & $-43.8\%$ \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:ablation} shows the end-to-end effect of the three remediations applied jointly: overall MASE improves from 3.030 to 1.704 (43.8\% relative), with improvements on all six datasets. The largest single gain is on exchange rate ($-63.3\%$), followed by ETTh2 ($-16.4\%$); the smallest are on ETTm1 ($-0.2\%$) and ETTh1 ($-0.7\%$), essentially at parity. The three fixes were validated jointly; the released v0.3 and v0.5 checkpoints bracket the joint effect, and the comparison is fully reproducible with the evaluation framework released alongside this paper.
...
Beyond point accuracy, we measure the empirical coverage of the predicted quantiles under the same protocol: for each test step we record whether the realized value falls at or below the predicted quantile, and average across steps, windows, and datasets. Table~\ref{tab:calibration} reports the results. Under this protocol, the v0.5 intervals are narrower than nominal: the nominal 80\% band (p10--p90) covers 51.3\% of held-out values, whereas the v0.3 band (from the same architecture trained with the original pipeline) covers 90.3\%. We therefore recommend treating v0.5 quantiles as \emph{relative} uncertainty signals (e.g., ranking forecast steps by uncertainty) rather than calibrated probabilities; the $p_{50}$ point forecast, which underlies all accuracy results in this paper, is unaffected.

\begin{table}[t]
\centering
\caption{Empirical quantile coverage under the standard protocol (mean across the six datasets), with nominal targets for reference. Both checkpoints share the identical architecture; the pipeline refinement improved point accuracy (Table~\ref{tab:ablation}) while narrowing the predicted intervals.}
...
\begin{axis}[
    ...
    ytick=data,
    yticklabels={NanoForecast (6.5M), PatchTST (15M+), TimesFM (200M), Chronos-T5-large (710M)},
    xmin=0,
    xmax=750,
    grid=major,
    grid style={dashed, gray!30},
...
\end{axis}
\end{tikzpicture}
\caption{Parameter count comparison. NanoForecast v0.5 (6.5M) is 31$\times$ smaller than TimesFM (200M) and 109$\times$ smaller than Chronos-T5-large (710M).}
\label{fig:params}
\end{figure}
...
\begin{itemize}
\item \textbf{Installation}: \texttt{pip install nanoforecast} from the GitHub repository (Apache 2.0).\footnote{The package is distributed via \url{https://github.com/eulogik/NanoForecast}; PyPI: \url{https://pypi.org/project/nanoforecast/}.}
\item \textbf{Custom training}: \texttt{train\_from\_csv.py --csv data.csv --target col --horizon H} trains on user-provided data.
\item \textbf{Model export}: ONNX export produces a 27.9\,MB FP32 model (9.2\,MB with INT8 quantization). The INT8 latency increase in Table~\ref{tab:latency} likely reflects dynamic quantization overhead on this CPU.
...
The DeltaNet recurrent state enables a mode uncommon among window-based transformer forecasters: stateful streaming inference. The model consumes new observations one at a time while preserving its recurrent memory across calls (no need to re-feed history), at the cost of one forward pass per update:
...
The 43.8\% improvement from pipeline refinement alone raises questions about reproducibility in time series research. Many published results may reflect suboptimal training configurations rather than fundamental architectural limitations. Our three issues are not architecture-specific---they can affect any model using multi-task loss functions, data augmentation, or mixed corpora. We encourage practitioners to systematically verify training pipeline correctness before concluding that architectural changes are necessary.

\subsection{Comparison with Foundation Models}
NanoForecast v0.5 does not match foundation model accuracy overall (MASE 1.704 vs.\ 1.447 for TimesFM and 1.554 for PatchTST, all re-evaluated under the identical standard protocol). This is expected: 6.5M parameters cannot match the representational capacity of 200M+ parameter models across all domains. However, the model outperforms TimesFM on all three ETT datasets and comes within 2\% on exchange rate; it outperforms PatchTST on all three ETT sets. Its losses are concentrated on high-cardinality multivariate sets (electricity, traffic), where breadth of pretraining dominates. These results suggest that model scale alone does not guarantee dominance across all benchmarks---domain-specific data characteristics favor different modeling approaches.
...
\begin{itemize}
\item \textbf{Accuracy gap}: Overall MASE (1.704 under the standard protocol) remains above the evaluated foundation models.
...
\begin{itemize}
\item GitHub: \url{https://github.com/eulogik/NanoForecast}
\item Model checkpoints (v0.3, v0.5): \url{https://huggingface.co/eulogik/nanoforecast-v05} and \url{https://huggingface.co/eulogik/nanoforecast-v03}
\item PatchTST baseline checkpoints: \url{https://huggingface.co/eulogik/nanoforecast-patchtst-baselines}
\item TimesFM checkpoint: \url{https://huggingface.co/google/timesfm-1.0-200m-pytorch}
\item Standard-protocol results (including quantile coverage metrics): \texttt{results/standard\_benchmark.json} in the GitHub repository (protocol string included in the file)
\item Interactive demo: \url{https://huggingface.co/spaces/eulogik/nanoforecast}
\item Training notebooks (Google Colab, free tier): \texttt{deploy/colab\_training\_v05.ipynb} in the repository
\end{itemize}
