...
4. **v0.6 — close the electricity/traffic gap** (prepared on branch `v0.6`): `colab_training_v05.ipynb` now trains per-dataset channel coverage (`channel_caps={"ETTh1":4,"ETTh2":4,"ETTm1":4,"exchange_rate":8,"electricity":64,"traffic":128}` with larger stride on big sets so the 321/862-channel sets are actually seen). v0.5 used a single `max_channels=4`, so it never trained on 317/321 electricity + 858/862 traffic channels — the diagnosed cause of the electricity/traffic loss. Re-run on T4 (resumable), then re-run `benchmark_standard.py`. Beating TimesFM on these two is not guaranteed (even PatchTST-15M loses there); expect to close the gap, not necessarily invert it.
